...
const EmployerContactInfoForm = ({ onPrevious, isLastStep }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    console.log(success);
    if (success) {
      navigate('/employer/accountcompletion')
    }
  }
    ...
    <div className="font-sans antialiased bg-gray-50 p-6 flex justify-center items-start">
      <div className="mx-auto bg-white rounded-lg shadow-md p-8 w-full">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Address Location */}
          <div>
            <label htmlFor="mapLocation" className="block text-sm font-medium text-gray-700 mb-1">Address Location</label>
          ...
          {/* Country */}
          <div>
            <label htmlFor="mapLocation" className="block text-sm font-medium text-gray-700 mb-1">Country</label>
            <div className="mt-1 flex rounded-md shadow-sm border border-solid border-gray-200">
              <span className="inline-flex items-center px-3 rounded-l-md border border-r-0 border-gray-300 bg-gray-50 text-gray-500 sm:text-sm">
                <i className="fa-solid fa-location-dot"></i>
              </span>
          ...
          <div className="mt-8 flex justify-start gap-4">
            {onPrevious && (
              <button
                type="button"
                onClick={onPrevious}
                className="bg-gray-600 hover:bg-gray-700 text-white font-medium py-2 px-6 rounded-md shadow-sm transition-colors duration-200 flex items-center space-x-2"
              >
            ...
            <button
              ...
              disabled={loading}
            >
              {isLastStep ? 'Complete' : 'Save Changes'}
            </button>
          </div>
        </form>
      </div>
...
export default EmployerContactInfoForm;
